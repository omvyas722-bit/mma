...
CREATE TABLE IF NOT EXISTS belt_levels (
  ...
  rank_order INTEGER NOT NULL UNIQUE, -- 1, 2, 3, 4, 5
  stripe_count INTEGER DEFAULT 0, -- 0-4 stripes per belt
  color_code TEXT, -- hex color for UI
  min_time_months INTEGER NOT NULL, -- minimum time at this level
  min_classes_attended INTEGER NOT NULL, -- minimum classes required
  description TEXT,
...
CREATE TABLE IF NOT EXISTS grading_requirements (
  ...
  category TEXT NOT NULL, -- 'striking', 'grappling', 'submissions', 'defense', 'conditioning'
...
CREATE TABLE IF NOT EXISTS member_belt_progress (
  ...
  is_current INTEGER DEFAULT 1, -- 1 for current belt, 0 for historical
  created_at DATETIME DEFAULT (datetime('now')),
...
CREATE TABLE IF NOT EXISTS member_techniques (
  ...
  proficiency_level TEXT DEFAULT 'learning', -- 'learning', 'practicing', 'proficient', 'mastered'
  ...
  FOREIGN KEY (assessed_by) REFERENCES staff(id)
);
...
CREATE TABLE IF NOT EXISTS grading_participants (
  ...
  score INTEGER, -- 0-100
  feedback TEXT,
  awarded_stripes INTEGER DEFAULT 0,
  created_at DATETIME DEFAULT (datetime('now')),
  updated_at DATETIME DEFAULT (datetime('now')),
  FOREIGN KEY (grading_session_id) REFERENCES grading_sessions(id),
  FOREIGN KEY (member_id) REFERENCES members(id),
  FOREIGN KEY (current_belt_id) REFERENCES belt_levels(id),
  FOREIGN KEY (testing_for_belt_id) REFERENCES belt_levels(id)
);
...
CREATE TABLE IF NOT EXISTS grading_history (
  ...
  stripes_awarded INTEGER DEFAULT 0,
...
-- Seed belt levels
INSERT INTO belt_levels (name, rank_order, stripe_count, color_code, min_time_months, min_classes_attended, description) VALUES
  ('White', 1, 4, '#FFFFFF', 0, 0, 'Beginner level - learning fundamentals'),
  ('Blue', 2, 4, '#0066CC', 6, 48, 'Intermediate level - developing technique'),
  ('Purple', 3, 4, '#6600CC', 18, 144, 'Advanced level - refining skills'),
  ('Brown', 4, 4, '#663300', 24, 192, 'Expert level - mastering techniques'),
  ('Black', 5, 0, '#000000', 36, 288, 'Master level - teaching and leading');

-- Seed grading requirements for White Belt
INSERT INTO grading_requirements (belt_level_id, category, technique_name, description, required, display_order) VALUES
