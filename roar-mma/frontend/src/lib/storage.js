...
// Set item
localStorage.set('user', { name: 'John', email: 'john@example.com' });

// Get item
const user = localStorage.get('user');

// Set with expiration (1 hour)
localStorage.set('token', 'abc123', { expires: 3600000 });
...
// Get all items
const items = localStorage.getAll();
...
storageKeys.authToken.set('abc123');
const token = storageKeys.authToken.get();
storageKeys.authToken.remove();

// Storage listener
import { StorageListener, localStorage } from './lib/storage';

const listener = new StorageListener(localStorage);

listener.on('user', (newValue, oldValue) => {
...
// Cache for 5 minutes
cache.set('api_response', data, 300000);
